...
async function loadData() {
    try {
        ...
        // Load transactions
        const transResult = await API.get(`/api/accounting/transactions/${currentDate}`);
        transactions = transResult.transactions || [];

        // Load balances
        const balResult = await API.get(`/api/accounting/balances/${currentDate}`);
...
async function showReceivablesDetails() {
    try {
        ...
        if (fuelReceivables.length === 0 && waterReceivables.length === 0) {
...
async function loadDebtItems() {
    try {
        ...
        if (debtType === 'fuel') {
            const result = await API.get(`/api/accounting/fuel-investments/${currentDate}`);
            ...
            const result = await API.get(`/api/accounting/water-investments/${currentDate}`);
            const investments = result.investments || [];
            const unpaid = investments.filter(inv => inv.payment_status !== 'paid');
